...
uint32_t fib(addr_t &main_stack, addr_t &f_stack, uint32_t n) {
  if (n == 0)
      return 0;
  if (n == 1)
      return 1;
  stack_saverestore(f_stack, main_stack);
  return fib(main_stack, f_stack, n - 1) + fib(main_stack, f_stack, n - 2);
}

uint32_t hanoi(addr_t &main_stack, addr_t &f_stack, uint32_t n) {
  if (n == 0)
      return 0;
  stack_saverestore(f_stack, main_stack);
  return hanoi(main_stack, f_stack, n - 1) + hanoi(main_stack, f_stack, n - 1) + 1;
}

void scheduler1(addr_t *pmain_stack, addr_t *pf_stack, fiber_t *fiber) {
  ...
  ret_val = ((uint32_t(*)(addr_t &, addr_t &, uint32_t))f)(main_stack, f_stack, arg1);
...
void shell_step_fiber_scheduler(shellstate_t &shellstate, addr_t main_stack, preempt_t preempt, addr_t stackptrs[], size_t stackptrs_size, addr_t arrays, size_t arrays_size, dev_lapic_t &lapic) {
  ...
  case START:
    ...
    case SCHEDULER_FIB:
      ...
      stack_init3(f_stack, f_array, f_arraysize, &scheduler1, &main_stack, &f_stack, &fiber);
      break;
